...
word f_pc = [
        # optimize 'mrmovq + rmmovq +jle',compute Cnd in advance
        D_icode == IJXX && D_ifun == CLE && E_icode == IRMMOVQ && M_icode == IMRMOVQ && !M_Cnd : D_valP;
...
word f_ifun = [
	imem_error : FNONE;
        f_icode == IMRMOVQ : CLE;     #to compute cnd in advance
	1: imem_ifun;
];
...
word e_valA =[
        E_icode == IRMMOVQ && E_srcA == M_dstM : m_valM;  #forwarding to elimate load/use harzard between mrmovq and rmmovq
...
bool D_bubble =
	...
	(E_icode == IJXX && !e_Cnd) &&
        # exclude Cnd forwarding
         !(E_ifun == CLE && M_icode == IRMMOVQ && W_icode == IMRMOVQ) ||
	# Stalling at fetch while ret passes through pipeline
