...
bool_t diff_mem(mem_t *oldm, mem_t *newm, FILE *outfile)   //wheather the two mem_t are different
{
    long_t pos;
    int len = oldm->len;
    bool_t diff = FALSE;
...
long_t get_reg_val(mem_t *r, regid_t id)    //get register value
{
    long_t val = 0;
    if (id >= REG_NONE)
        return 0;
...
void set_reg_val(mem_t *r, regid_t id, long_t val)  //set register value; r is an array which store all the registers
{
    if (id < REG_NONE)
...
stat_t nexti(y64sim_t *sim)
{
    ...
    itype_t icode;      //type of instruction 
    alu_t ifun;   
    long_t immediate;
    long_t numOfregA;
    long_t numOfregB;
    ...
    switch (icode) {
      ...
      case I_RRMOVQ:  /* 2:x regA:regB */
        if (cond_doit(sim->cc,icond)){
            numOfregA = get_reg_val(sim->r,regA);
            set_reg_val(sim->r,regB,numOfregA);
        }
        sim->pc = next_pc;
        break;
      ...
      case I_RMMOVQ: /* 4:0 regA:regB imm */
        num = get_reg_val(sim->r,regA);
        dest = get_reg_val(sim->r,regB);
        dest += immediate;
        if (dest<0||dest>sim->m->len){
            err_print("PC = 0x%lx, Invalid data address 0x%lx", sim->pc,dest);
            return STAT_ADR;
        }
        set_long_val(sim->m,dest,num);
        sim->pc=next_pc;
        break; 
      case I_MRMOVQ: /* 5:0 regB:regA imm */
        ...
        get_long_val(sim->m,dest,&num);
        set_reg_val(sim->r,regA,num);
        sim->pc=next_pc; 
        break;
      case I_ALU: /* 6:x regA:regB */
        numOfregA=get_reg_val(sim->r,regA);
        numOfregB=get_reg_val(sim->r,regB);
        num=compute_alu(ifun,numOfregA,numOfregB);
        sim->cc=compute_cc(ifun,numOfregA,numOfregB,num);
      ...
      case I_JMP: /* 7:x imm */
        ...
        break;
      case I_CALL: /* 8:x imm */
        num = get_reg_val(sim->r,REG_RSP);
        num-=8;
        set_reg_val(sim->r,REG_RSP,num);
        if (num<0||num>sim->m->len){
            err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,num);
            return STAT_ADR;
        }
        set_long_val(sim->m,num,next_pc);
        sim->pc = immediate;
        break;
      case I_RET: /* 9:0 */
        num = get_reg_val(sim->r,REG_RSP);
        get_long_val(sim->m,num,&immediate);
        num+=8;
        set_reg_val(sim->r,REG_RSP,num);
        sim->pc=immediate;
        break;
      case I_PUSHQ: /* A:0 regA:F */
        ...
        if (regA == REG_RSP){
            set_long_val(sim->m,num,num+8);
        }
        else{
            numOfregA = get_reg_val(sim->r,regA);
            set_long_val(sim->m,num,numOfregA);
            /*set_reg_val(sim->r,REG_RSP,num);*/
        }
        sim->pc = next_pc;
        break;
      case I_POPQ: /* B:0 regA:F */
        num = get_reg_val(sim->r,REG_RSP);
        get_long_val(sim->m,num,&numOfregA);
        num+=8;
        set_reg_val(sim->r,REG_RSP,num);
        set_reg_val(sim->r,regA,numOfregA);
        sim->pc = next_pc;
        break;
